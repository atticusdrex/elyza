...
LENGTH_SCALE, KL_TERMS, GRF_MEAN, GRF_STD = 0.2, 64, 0.0, 1.0 
...
lf_darcy = DarcyFlowEvaluator(name='low-fidelity darcy flow', inputs=[lf_perm], grid_dim=LF_DIM, cost=1.4149e-04, source_term = "figs/Pi.jpg")

mf_darcy = DarcyFlowEvaluator(name='medium-fidelity darcy flow', inputs=[mf_perm], grid_dim=MF_DIM, cost=2.8317e-03, source_term = "figs/Pi.jpg")

hf_darcy = DarcyFlowEvaluator(name='medium-fidelity darcy flow', inputs=[hf_perm], grid_dim=HF_DIM, cost=4.4440e-02, source_term = "figs/Pi.jpg")
...
axes[0,0].set_ylabel("High-Fidelity", fontsize=14)
axes[0,0].set_xticks([]); axes[0,0].set_yticks([])

im_force = axes[0,1].imshow(force_hf, cmap = 'inferno', **force_norm)
...
axes[1,0].set_ylabel("Medium-Fidelity", fontsize=14)
axes[1,0].set_xticks([]); axes[1,0].set_yticks([])

axes[1,1].imshow(mf_darcy._f.reshape((MF_DIM, MF_DIM)), cmap = 'inferno', **force_norm)
...
axes[2,0].set_ylabel("Low-Fidelity", fontsize=14)
axes[2,0].set_xticks([]); axes[2,0].set_yticks([])

axes[2,1].imshow(lf_darcy._f.reshape((LF_DIM, LF_DIM)), cmap = 'inferno', **force_norm)
...
# %% 
rmfmc.get_pilots(jrand.PRNGKey(42), n_pilots = 100, set_costs = False)
...
figure(figsize=(16,4))
subplot(1,3,1)
imshow(rmfmc_var, cmap = "Oranges", vmin=0.0, vmax = hfmc_var.max())
colorbar()
title("R-MFMC Variance")
xticks([]); yticks([])

subplot(1,3,2)
imshow(hfmc_var, cmap = "Oranges", vmin=0.0, vmax = hfmc_var.max())
colorbar()
title("HFMC Variance")
xticks([]); yticks([])
