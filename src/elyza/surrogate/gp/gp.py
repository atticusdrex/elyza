"""Vanilla Gaussian Process regression.

Defines :class:`GaussianProcess`, a :class:`~elyza.surrogate.abstract.Surrogate`
that combines a pluggable :class:`~elyza.surrogate.gp.kernel.BaseKernel` and
:class:`~elyza.surrogate.gp.mean.BaseMean`, fits hyperparameters via a
pluggable :class:`~elyza.optim.abstract.Optimizer` on the negative
log-marginal-likelihood, and supports rank-1 (block Cholesky) updates for
